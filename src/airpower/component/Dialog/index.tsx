...
const Dialog: React.FC<DialogPropTypes> = ({
  ...
  function onMouseMove(e: SyntheticEvent) {
    if (isDragging) {
      ...
      if (mouseNativeEvent.clientX - startX <= thresholdDistance) {
        ...
        mouseNativeEvent.clientX - startX >=
        window.innerWidth - getWidthNumber(width) - thresholdDistance
      ) {
      ...
      if (mouseNativeEvent.clientY - startY <= thresholdDistance) {
        ...
        mouseNativeEvent.clientY - startY >=
        window.innerHeight - getHeightNumber(height) - thresholdDistance
      ) {
  ...
  const [top, setTop] = useState(
    window.innerHeight / 2 - getHeightNumber(height) / 2,
  )

  const [left, setLeft] = useState(
    window.innerWidth / 2 - getWidthNumber(width) / 2,
  )
    ...
    >
      <div
        ...
        style={{
          ...
          top,
          left,
          transition: isDragging ? 'none' : 'all .3s',
        }}
      >
        <div
          className="header"
        ...
        <Spin spinning={loading} wrapperClassName="spin-wrapper">
          <div className="dialog_body">{props.children}</div>
          {!props.hideFooter ? (
            <div className="footer">
              {props.footer ? (
                ...
                <>
                  {!props.hideConfirm ? (
                    <Button type="primary" onClick={_onConfirm}>
                      {confirmText}
                    </Button>
                  ) : (
                    ''
